...
module.exports = function(path, document, cb) {
  ...
  mailparser.on("end", function(mail_object){
    ...
    if (mail_object.headers && mail_object.headers.date) {
      document.creation_date = Date.parse(mail_object.headers.date);
    }
    ...
    var cluestr = new anyfetchClient();
    ...
    async.each((mail_object.attachments || []), function(attachment, cb){
      ...
      docAttachment.creation_date = Date.parse(mail_object.headers.date);
      // File to send
      var fileConfig = {
      ...
      cluestr.sendDocumentAndFile(docAttachment, fileConfig, function(err) {
        cb(err);
      });
    }, function(err){
      cb(err, document);
