...
module.exports = function(path, document, cb) {
  ...
  mailparser.on("end", function(mail_object){
    ...
    async.each((mail_object.attachments || []), function(attachment, cb) {
      ...
      if (!mail_object.html || document.datas.html.indexOf('src="cid:' + fn) === -1) {
        ...
        var image = new Buffer(attachment.content);

        document.datas.html = document.datas.html.replace(new RegExp('src="cid:' + fn + '[^"]*\"','g'), 'src=data:image/' + extension + ';base64,' + base64_encode(image));
        cb();
      }
    }, function(err){
      cb(err, document);
